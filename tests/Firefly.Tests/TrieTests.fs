module Firefly.Tests.TrieTests
...
let ``Trie distinguishes between methods on same path`` () =
    ...
    let struct(hGet, _) = (Trie.lookup "GET" "/items" trie).Value
    let struct(hPost, _) = (Trie.lookup "POST" "/items" trie).Value
...
let ``Trie static segment takes priority over param`` () =
    ...
    let struct(hMe, _) = (Trie.lookup "GET" "/users/me" trie).Value
    let struct(hParam, _) = (Trie.lookup "GET" "/users/42" trie).Value
...
let ``Trie add wildcard with existing wildcard adds method`` () =
    ...
    getResult |> ValueOption.isSome |> should be True
    postResult |> ValueOption.isSome |> should be True
...
let ``Trie pre-composes middleware chain`` () =
    ...
    let struct(h, _) = (Trie.lookup "GET" "/test" trie).Value
...
let ``Trie routes with different param names at same level resolve correctly`` () =
    ...
    runResult |> ValueOption.isSome |> should be True
    let struct(hRun, psRun) = runResult.Value
    ...
    signalResult |> ValueOption.isSome |> should be True
    let struct(hSignal, psSignal) = signalResult.Value
...
let ``Trie reuses existing param child node`` () =
    ...
    let struct(hGet, ps1) = (Trie.lookup "GET" "/items/42" trie).Value
    let struct(hPost, ps2) = (Trie.lookup "POST" "/items/42" trie).Value
