...
Fire is part of a cohesive set of F# libraries designed to work together.

## Overview

| Library | Purpose | Key Feature |
|---------|---------|-------------|
| **Fire** | Web framework | Routing, middleware, DI, gRPC |
...
Use Flare as an HTTP client inside Fire handlers, with DI for service registration:
...
Evlog provides structured logging that integrates with Fire's request lifecycle:

```fsharp
// Fire's Evlog module is available for logging within handlers
...
Fire has a direct dependency on Flame for the `Schema` module. The others are optional and integrated via DI.
