...
Fire integrates with the built-in .NET dependency injection container. Services registered via `App.services` are available for auto-injection into route handlers.
...
When a handler parameter is an interface or abstract type, Fire automatically resolves it from the DI container. No attributes or special syntax needed:
...
The order of parameters in the function signature does not matter for DI vs route params -- Fire classifies each parameter by its type:
